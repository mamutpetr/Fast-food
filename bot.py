...
DOSAGE_DATA = {
    "ptsd_insomnia": {"name": "Легкий перекус", "doses": {50: 78, 60: 85, 70: 93, 80: 100, 90: 108, 100: 115, 110: 123, 120: 130}},
    "pain": {"name": "Ситний обід", "doses": {50: 91, 60: 99, 70: 106, 80: 113, 90: 120, 100: 128, 110: 135, 120: 142}},
    "stress": {"name": "Вечеря на двох", "doses": {50: 64, 60: 68, 70: 73, 80: 77, 90: 82, 100: 87, 110: 91, 120: 95}},
    "depression": {"name": "Свято з друзями", "doses": {50: 76, 60: 88, 70: 99, 80: 111, 90: 122, 100: 133, 110: 145, 120: 156}},
    "migraine": {"name": "Швидкий сніданок", "doses": {50: 85, 60: 87, 70: 90, 80: 93, 90: 96, 100: 99, 110: 102, 120: 105}},
    "epilepsy": {"name": "Максимальний голод", "doses": {50: 174, 60: 210, 70: 245, 80: 280, 90: 315, 100: 350, 110: 385, 120: 420}}
}
...
def display_profile(message, phone, game_discount):
    ...
    text = (f"👤 **Твій кабінет Burger Chef**\n\n"
            f"🏷 Статус: *{group_name}*\n"
            f"📱 Телефон: `{phone}`\n\n"
...
# --- КАЛЬКУЛЯТОР ДОЗИ ---
@bot.message_handler(func=lambda m: m.text == "🧮 Калькулятор ситості")
def calc_start(message):
    ...
    bot.send_message(message.chat.id, "🍽 **Крок 1/3:** Оберіть ваш рівень голоду:", reply_markup=markup, parse_mode="Markdown")
...
def calc_weight(call):
    ...
    bot.edit_message_text("⚖️ **Крок 2/3:** Оберіть вашу вагу тіла (для розрахунку калорій):", call.message.chat.id, call.message.message_id, reply_markup=markup, parse_mode="Markdown")
...
def calc_conc(call):
    ...
    text = f"🎯 Ваша орієнтовна норма: **{dose}0 калорій**.\n\n🧪 **Крок 3/3:** Оберіть рівень гостроти страв (%):"
...
def send_invoice(chat_id):
    ...
    bot.send_invoice(chat_id, "Burger Chef", "Оплата замовлення (Доставка)", payload, PAYMENT_TOKEN, "UAH", prices, need_phone_number=True, need_shipping_address=False)
...
def process_broadcast(message):
    ...
    count = 0
    for u in users:
        try: bot.send_message(u[0], f"📢 **Новина Burger Chef:**\n\n{message.text}", parse_mode="Markdown"); count += 1
        except: pass
